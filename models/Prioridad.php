<?php
    class Prioridad extends Conectar{
        ...
        public function get_prioridad(){
            ...
            parent::set_names();
            $sql="SELECT * FROM atn_prioridad WHERE est=1;";
        ...
        public function insert_prioridad($prio_nom){
            $conectar= parent::conexion();
            parent::set_names();
            $sql="INSERT INTO atn_prioridad (prio_id, prio_nom, est) VALUES (NULL,?,'1');";
            $sql=$conectar->prepare($sql);
        ...
        public function get_prioridad_x_id($prio_id){
            $conectar= parent::conexion();
            parent::set_names();
            $sql="SELECT * FROM atn_prioridad WHERE prio_id = ?";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $prio_id);
            $sql->execute();
            return $resultado=$sql->fetchAll();
